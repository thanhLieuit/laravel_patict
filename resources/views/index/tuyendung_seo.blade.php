		...
		<div class="row page-titles">
	        <div class="col-md-6 align-self-center">  
	            <ul class="breadcrumb" style="background-color: #e9ecef00;">
	                ...
	                <li style="padding-left: 10px;padding-right: 10px;"><i class='fas fa-angle-double-right'></i></li>
	                <li class="">{{ trans('messages.recruitment4') }}</li>
	            </ul>
	        </div>
			...
			<div class="col-md-6">
				<div class="ndtuyendungdesign">
					<div class="titledesign">
						...
						<p>{{ trans('messages.pseo1-1') }}</p>
						<p>{{ trans('messages.pseo1-2') }}</p>
						<p>{{ trans('messages.pseo1-3') }}</p>
						<p>{{ trans('messages.pseo1-4') }}</p>
						<p>{{ trans('messages.pseo1-5') }}</p>
						<p>{{ trans('messages.pseo1-6') }}</p>
						<br>
						<h3>{{ trans('messages.title2') }}</h3>
						<p>{{ trans('messages.p2-1') }}	</p>
						<p>{{ trans('messages.pseo2-2') }}</p>
						<p>{{ trans('messages.pseo2-3') }}</p>
						<p>{{ trans('messages.pseo2-4') }}</p>
						<p>{{ trans('messages.pseo2-5') }}</p>
						<p>{{ trans('messages.pseo2-6') }}</p>
						<p>{{ trans('messages.pseo2-7') }}</p>
						<p>{{ trans('messages.pseo2-8') }}</p>
				...
				<div class="designdk">
					<form action="" enctype="multipart/form-data" method="POST">
							...
							<div class="row">
								<div class="col-md-12">
									<div class="radio" style="float: left;margin-right: 20px;">
						    			<input type="radio" name="optradio" value="seo">SEO
						    		</div>
								</div>
						...
						<div class="form-group knlm">	
							<label><h5>{{ trans('messages.formseo-3') }} (<strong style="color: red">*</strong>) </h5></label><br>
							<textarea class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="Word? Exel? Google Analytics? Google Webmaster Tools?" name="congcu" value="{{ old('congcu') }}"></textarea>
						</div>
						<div class="form-group knlm">
