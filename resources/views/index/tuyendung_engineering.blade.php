...
@section('content')
@section('title','Tuyển dụng Design & Ux')
<section id="banner">
		...
		<div class="row page-titles">
	        <div class="col-md-6 align-self-center">  
	            <ul class="breadcrumb" style="background-color: #e9ecef00;">
	                ...
	                <li style="padding-left: 10px;padding-right: 10px;"><i class='fas fa-angle-double-right'></i></li>
	                <li class="">{{ trans('messages.recruitment2') }}</li>
	            </ul>
	        </div>
			...
			<div class="col-md-6">
				<div class="ndtuyendungdesign">
					<div class="titledesign">
						...
						<p>{{ trans('messages.pks1-1') }}</p>
						<p>{{ trans('messages.pks1-2') }}</p>
						<p>{{ trans('messages.pks1-3') }}</p>
						<p>{{ trans('messages.pks1-4') }}</p>
						<p>{{ trans('messages.pks1-5') }}</p>
						<br>
						<h3>{{ trans('messages.title2') }}</h3>
						<p>{{ trans('messages.p2-1') }}	</p>
						<p>{{ trans('messages.pks2-2') }}</p>
						<p>{{ trans('messages.pks2-3') }}</p>
						<p>{{ trans('messages.pks2-4') }}</p>
						<p>{{ trans('messages.pks2-5') }}</p>
						<p>{{ trans('messages.p2-4') }}</p>
			...
			<div class="col-md-6">
				<div class="designdk">
					<form action="" enctype="multipart/form-data" method="POST">
							...
							<div class="row">
								<div class="col-md-6">
									<div class="radio" style="float: left;margin-right: 20px;">
						    			<input type="radio" name="optradio" value="Web Developer">Web Developer
						    		</div>
								</div>
								<div class="col-md-6">
									<div class="radio">
						     			<input type="radio" name="optradio" value="IOS App Developer">IOS App Developer
						    		</div>
								</div>
							</div>    
						</div>
						<div class="form-group knlm">	
							<label><h5>{{ trans('messages.formks-3') }} (<strong style="color: red">*</strong>) </h5></label><br>
							<textarea class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="{{ trans('messages.f3ks-nd') }}" name="congcu" value="{{ old('congcu') }}"></textarea>
						</div>
						<div class="form-group knlm">
							<label for=""><h5>{{ trans('messages.form-4') }} (<strong style="color: red">*</strong>) </h5></label><br>
							<textarea class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="{{ trans('messages.f4-nd') }}" name="kinhnghiem" value="{{ old('kinhnghiem') }}"></textarea>
						</div>
						<div class="form-group knlm">
						...
						<button type="submit" class="btn btn-success" data-toggle="modal" data-target="#myModal">{{ trans('messages.btn-ut') }}</button>
					</form>
				</div>
			</div>
		</div>
...
<section id="chuyenbai">
	<div class="container">
		<div class="row">
			<div class="col-md-6">
				<a href="https://patict.vn/tuyen-dung-design">
					<div class="traicb">
						<i class='fas fa-angle-double-left' style=''></i>
						<p style="">{{ trans('messages.ct-front1') }}</p>
					</div>
				</a>
				
			</div>
			<div class="col-md-6">
				<a href="https://patict.vn/tuyen-dung-sale-marketing">
					<div class="phaicb1" >
						<p style="">{{ trans('messages.ct-back1') }}</p>
